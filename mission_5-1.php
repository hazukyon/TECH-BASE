...
$sql = "SELECT * FROM msg WHERE id='$edit' ";//変更する投稿番号を選択
	...
	if($editpass == $pass){//投稿フォームに該当の名前、コメントを表示
		$newname = $row['name'];
...
$id = $editver; //変更する投稿番号
	...
	$nowtime = date("Y/m/d H:i:s");
	$pass = $_POST["pass"];
	$sql = 'update msg set name=:name,comment=:comment, nowtime=:nowtime, pass=:pass where id=:id';//編集実行
	$stmt = $pdo->prepare($sql);
	...
	if(empty($_POST["pass"])){//パスワード再入力忘れのとき、再表示
		echo "パスワードを入れてください";
		$edit = $_POST["edit"];
		$editpass = $_POST["edipass"];
		$sql = "SELECT * FROM msg WHERE id='$edit' ";//変更する投稿番号を選択
